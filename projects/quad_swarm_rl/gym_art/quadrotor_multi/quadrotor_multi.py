...
#import sys
#external_libs_path = '/home/tianhao/Desktop/final/open-sbt-core-fork-thesis'
#sys.path.append(external_libs_path)
#from search_swarm2 import SwarmSimulator

#from gym_art import global_variable
...
class QuadrotorEnvMulti(gym.Env):
    ...
    def obst_generation_given_density(self, grid_size=1.0):
        ...
        #默认情况下density是0.2,这里是向下取了整
        
        obst_pos_arr = []
        # 0: No Obst, 1: Obst
        obst_map = np.zeros([obst_area_length, obst_area_width])

        for obst_id in [int(os.environ.get("OBST1_INDEX")), int(os.environ.get("OBST2_INDEX"))]:
        ...
        QuadrotorEnvMulti.obs_arr = obst_pos_arr    
    ...
    @staticmethod
    def calculate_min_distance(pos_arr):
        min_distance = float('inf')  # 初始设为无限大
        
        # 遍历所有无人机组合
        for drone1 in pos_arr:
            for drone2 in pos_arr:
                if drone1 < drone2:  # 避免重复计算同一对
                    # 获取较短的时间步长以避免越界
                    num_steps = min(len(pos_arr[drone1]), len(pos_arr[drone2]))
                    
                    # 遍历每个时刻
                    for t in range(num_steps):
                        pos1 = pos_arr[drone1][t]  # 获取无人机1在第t个时刻的坐标
                        pos2 = pos_arr[drone2][t]  # 获取无人机2在第t个时刻的坐标
                        
                        # 计算这两个位置的距离
                        distance = QuadrotorEnvMulti.euclidean_distance(pos1, pos2)
    ...
    @staticmethod
    def calculate_min_distance_with_obsts(pos_dict, obst_arr):
        obst_size = float(os.environ.get("quads_obst_size", 0))
        half_obst_size = obst_size / 2

        min_distance = float('inf')

        for drone_id, path in pos_dict.items():
            for pos in path:
                x, y, _ = pos
                for obst in obst_arr:
                    obst_x, obst_y, _ = obst
                    distance = np.sqrt((x - obst_x) ** 2 + (y - obst_y) ** 2) - half_obst_size
                    if distance < min_distance:
                        min_distance = distance
    ...
    @staticmethod
    def calculate_min_distance_with_obst1(pos_dict, obst_arr):
        def distance(point, target):
            return math.sqrt((point[0] - target[0])**2 + (point[1] - target[1])**2)
        
        closest_point = min(obst_arr, key=lambda p: distance(p, [3.5, -3.5]))
    ...
    @staticmethod
    def calculate_min_distance_with_obst1_2(pos_dict, obst_arr):
        def distance(point, target):
            return math.sqrt((point[0] - target[0])**2 + (point[1] - target[1])**2)
    ...
    @staticmethod
    def calculate_min_distance_with_obst2(pos_dict, obst_arr):
        def distance(point, target):
            return math.sqrt((point[0] - target[0])**2 + (point[1] - target[1])**2)
        
        farthest_point = max(obst_arr, key=lambda p: distance(p, [3.5, -3.5]))
    ...
    @staticmethod
    def calculate_min_distance_with_obst2_2(pos_dict, obst_arr):
        def distance(point, target):
            return math.sqrt((point[0] - target[0])**2 + (point[1] - target[1])**2)
    ...
    @staticmethod
    def calculate_min_distance_to_floor(pos_dict):
        floor_z = -5
        min_distance = float('inf')
    ...
    @staticmethod
    def calculate_min_distance_to_ceiling(pos_dict):
        floor_z = 5
        min_distance = float('inf')
    ...
    @staticmethod
    def calculate_min_distance_to_walls(pos_dict):
        bounds = [5, -5]
        min_distance = float('inf')
        ...
        for drone_id, path in pos_dict.items():
            for pos in path:
                x, y, z = pos
                for bound in bounds:
                    distance_to_x_bound = abs(x - bound)
                    distance_to_y_bound = abs(y - bound)
                    
                    # Update minimum distance and coordinates
                    if distance_to_x_bound < min_distance:
                        min_distance = distance_to_x_bound
                    if distance_to_y_bound < min_distance:
                        min_distance = distance_to_y_bound

        return min_distance
    
    @staticmethod
    def calculate_max_distance_between_drones(pos_dict):
        # Extract the last 20% of the path for each drone
        last_20_percent_paths = {drone_id: path[int(len(path) * 0.8):] for drone_id, path in pos_dict.items()}
        
        max_distance = 0

        # Iterate over each pair of drones
        drone_ids = list(last_20_percent_paths.keys())
        for i in range(len(drone_ids)):
            for j in range(i + 1, len(drone_ids)):
                path_i = last_20_percent_paths[drone_ids[i]]
                path_j = last_20_percent_paths[drone_ids[j]]
                
                # Calculate the distance between each pair of positions in the last 20% of the paths
                for pos_i in path_i:
                    for pos_j in path_j:
    ...
    def calculate_max_distance_to_goal(pos_dict, target_point):
    # Extract the last 20% of the path for each drone
        last_20_percent_paths = {drone_id: path[int(len(path) * 0.8):] for drone_id, path in pos_dict.items()}
        ...
        # Iterate over each drone's path
        for drone_id, path in last_20_percent_paths.items():
            for pos in path:
                pos = np.array(pos)
                distance = np.linalg.norm(pos - target_point)
                if distance > max_distance:
                    max_distance = distance
    ...
    @staticmethod
    def visualize_drones_positions(pos_dict, obst_arr, obst_size, target):
        # 获取保存路径并创建所需的子文件夹
        base_dir = os.environ.get("PATH_TO_SAVE")
        gen_num_dir = os.environ.get("GENERATION_NUM")
        
        # 确保生成的目录存在
        output_dir = os.path.join(base_dir, gen_num_dir)
        ...
        # 获取环境变量中的障碍物索引和大小
        obst_indexes = [float(os.environ.get(f"OBST{i+1}_INDEX")) for i in range(2)]
        quads_obst_size = float(os.environ.get("quads_obst_size"))
        ...
        for drone_id, path in pos_dict.items():
            path = np.array(path)
            ax.plot(path[:, 0], path[:, 1], path[:, 2], color=colors[drone_id % len(colors)], label=f'Drone {drone_id}')
            # Mark starting position
            ax.scatter(path[0, 0], path[0, 1], path[0, 2], color=colors[drone_id % len(colors)], marker='o', s=100, label=f'Start {drone_id}')
        ...
        for obst in obst_arr:
            ...
            x_grid = x + (obst_size / 2) * np.cos(theta_grid)
            y_grid = y + (obst_size / 2) * np.sin(theta_grid)
        ...
        file_name_3d = os.path.join(output_dir, f'drones_3D_{current_time}_{obst_indexes}_{quads_obst_size}.png')
        plt.savefig(file_name_3d, bbox_inches='tight')
        plt.close()  
        ...
        for drone_id, path in pos_dict.items():
            path = np.array(path)
            ax.plot(path[:, 0], path[:, 1], color=colors[drone_id % len(colors)], label=f'Drone {drone_id}')
            # Mark starting position
            ax.scatter(path[0, 0], path[0, 1], color=colors[drone_id % len(colors)], marker='o', s=100, label=f'Start {drone_id}')
        
        # Plot obstacles
        for obst in obst_arr:
            circle = plt.Circle((obst[0], obst[1]), obst_size / 2, color='gray', alpha=0.5)
            ax.add_patch(circle)
        ...
        file_name_2d = os.path.join(output_dir, f'drones_2D_{current_time}_{obst_indexes}_{quads_obst_size}.png')
        plt.savefig(file_name_2d, bbox_inches='tight')
        plt.close()
    ...
    def reset(self, obst_density=None, obst_size=None):
        ...
        if(QuadrotorEnvMulti.has_executed_reset):
            QuadrotorEnvMulti.visualize_drones_positions(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr, float(os.environ.get("quads_obst_size")), [0.5, -3.5, 2.25])
    ...
    def step(self, actions):
        ...
        if any(dones):
            ...
            num_collisions_after_settle = self.obst_quad_collisions_after_settle
            ...
            min_distance = QuadrotorEnvMulti.calculate_min_distance(QuadrotorEnvMulti.pos_dict)
            min_distance_with_obsts = QuadrotorEnvMulti.calculate_min_distance_with_obsts(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr)
            min_distance_with_obst1 = QuadrotorEnvMulti.calculate_min_distance_with_obst1(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr)
            average_distance_with_obsts = (QuadrotorEnvMulti.calculate_min_distance_with_obst1(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr) + QuadrotorEnvMulti.calculate_min_distance_with_obst2(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr)) / 2.
            average_distance_with_obsts_2 = (QuadrotorEnvMulti.calculate_min_distance_with_obst1_2(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr) + QuadrotorEnvMulti.calculate_min_distance_with_obst2_2(QuadrotorEnvMulti.pos_dict, QuadrotorEnvMulti.obs_arr)) / 2.


            min_distance_to_floor = QuadrotorEnvMulti.calculate_min_distance_to_floor(QuadrotorEnvMulti.pos_dict)
            min_distance_to_ceiling = QuadrotorEnvMulti.calculate_min_distance_to_ceiling(QuadrotorEnvMulti.pos_dict)
            min_distance_to_walls = QuadrotorEnvMulti.calculate_min_distance_to_walls(QuadrotorEnvMulti.pos_dict)
            max_distance_between_drones = QuadrotorEnvMulti.calculate_max_distance_between_drones(QuadrotorEnvMulti.pos_dict)
            max_distance_to_goal = QuadrotorEnvMulti.calculate_max_distance_to_goal(QuadrotorEnvMulti.pos_dict, [3.5, -3.5, 2.25])
            ...
            data_to_log["statistics"]['min_distance'] = [min_distance]
            data_to_log["statistics"]['min_distance_with_obsts'] = [min_distance_with_obsts]
            data_to_log["statistics"]['min_distance_to_floor'] = [min_distance_to_floor]
            data_to_log["statistics"]['min_distance_to_ceiling'] = [min_distance_to_ceiling]
            data_to_log["statistics"]['min_distance_to_walls'] = [min_distance_to_walls]
            data_to_log["statistics"]['max_distance_between_drones_after_settle'] = [max_distance_between_drones]
            data_to_log["statistics"]['max_distance_to_goal'] = [max_distance_to_goal]
            data_to_log["statistics"]['min_distance_with_obst1'] = [min_distance_with_obst1]
            data_to_log["statistics"]['average_distance_with_obsts'] = [average_distance_with_obsts]
            data_to_log["statistics"]['average_distance_with_obsts_2'] = [average_distance_with_obsts_2]
