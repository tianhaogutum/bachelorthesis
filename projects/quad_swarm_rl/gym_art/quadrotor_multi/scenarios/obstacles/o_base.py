...
class Scenario_o_base(QuadrotorScenario):
    ...
    def max_square_area_center(self):
        ...
        #pos_x_str = os.environ.get("POS_X_END_POINT")     
        #pos_x = float(pos_x_str)  # 或者使用 int(pos_x_str) 如果你需要整数

        #这里改变了也相应改变画图的！
        #pos_x = float(os.environ.get("POS_X_END_POINT"))
        pos_x = 0.5
        pos_y = -3.5
        #z_list_start = float(os.environ.get("POS_Z_END_POINT"))
        z_list_start = 2.25
        return np.array([pos_x, pos_y, z_list_start])
        #return np.array([-3.5, -3.5, 1.5])
